...
| 端点 | 方法 | 说明 |
|------|------|------|
| `POST /api/query` | POST | 提交 NL 查询 → SSE 流式。可选 `data_source_id` 指定目标数据源；不传则回退到 `is_active=true` 的激活源。 |
...
{
  "text": "用户上传了哪些文档",
  "data_source_id": "4fe7cc7e-..."   // 可选；不传则使用激活源
}
```

- `data_source_id` 不存在 → 404
- `data_source_id` 为空字符串/不传 → 回退查 `is_active=true`
...
- 第 4 轮（2026-06-21）：`POST /api/query` 新增可选 `data_source_id` 字段。方案 B（可选，fallback 到激活源）。`data_source_id` 不存在时返回 404。