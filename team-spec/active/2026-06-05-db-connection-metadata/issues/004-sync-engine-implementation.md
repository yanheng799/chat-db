...
- [ ] APScheduler 进程内调度，按 `METADATA_SYNC_INTERVAL_HOURS` 配置间隔（默认 24 小时）执行定时全量同步
- [ ] `POST /api/datasources/{id}/sync` 手动触发同步（后台异步执行）
- [ ] 不传 `table_scope` 时全量同步（检查所有表的 schema 变更）
- [ ] 传 `table_scope` 时仅检查指定表的字段/索引/外键变更（增量检查）
- [ ] 差异更新逻辑：对比目标库 information_schema 与已存元数据，检测 table_added / table_removed / column_added / column_removed / column_modified / index_added / index_removed / fk_added / fk_removed
- [ ] 先写 `metadata_change_logs`，再更新元数据表
- [ ] `metadata_sync_logs` 记录 scope 字段（null=全量，数组=表级），以及 tables_added / tables_removed / columns_changed 统计
- [ ] 同一 data_source 同一时刻仅允许一个 status=running 的 sync_log，冲突时返回 HTTP 409
- [ ] 同步失败时记录 error_message，status=failed，不影响已存元数据
...
- [ ] Given 同步正在运行，When 再次触发同步，Then 返回 HTTP 409
- [ ] Given 手动同步指定 table_scope=[orders]，When 目标库 orders 表新增 1 个字段，Then 仅检查 orders 表，change_log 有 column_added 记录
- [ ] 同步引擎有对应的 pytest 测试，diff 计算和并发防护有独立测试
